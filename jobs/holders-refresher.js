...
import { updateWalletPortfolio } from '../core/portfolio.js';

try {
  console.log("[holders/portfolio] updating", addr);
  await updateWalletPortfolio(addr);
} catch (e) {
  console.error("[holders/portfolio] error:", e.message);
}
  ...
  for (let i = 0; i < maxPages; i++) {
    ...
    try {
      ...
      for (const it of items) {
        ...
        info('[holders/portfolio] updating', addr);
        await updateWalletPortfolio(addr).catch(e => warn('[holders/portfolio]', e.message));
      }
